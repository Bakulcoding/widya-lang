...
// ---------------------------------------------------------------------------
// ServerHttp (T3) - manajemen server Widya + handler function non-Send
// ---------------------------------------------------------------------------
...
/// Menyimpan state server Widya (rute, static folder) untuk satu port.
/// Tidak Send, jadi hanya boleh diakses di thread utama interpreter yang sama.
/// Ketika `jalankan` dipanggil, kita akan menyalin AST function ke struct yang Send (via ValueSerial)
/// dan menyerahkannya ke WebServer yang berjalan di thread baru.
struct ServerHandle {
    port: u16,
    mode: String, // "tunggal" | "utas"
    status: String,
    rute_get: HashMap<String, Value>,  // path -> Value::Function
...
// ---------------------------------------------------------------------------
// Builtin functions untuk ServerHttp
// ---------------------------------------------------------------------------

pub fn daftarkan_serverweb(env: &mut crate::environment::Environment) {
    use crate::value::BuiltinFunction;
    
    let builtin = BuiltinFunction {
        name: "ServerHttp".to_string(),
        arity: Some(1),
        func: builtin_serverweb_http,
    };
    env.define("ServerHttp".to_string(), Value::Builtin(Rc::new(builtin)), true);

    let builtin = BuiltinFunction {
        name: "tambah_rute".to_string(),
        arity: Some(4),
        func: builtin_tambah_rute,
    };
    env.define("tambah_rute".to_string(), Value::Builtin(Rc::new(builtin)), true);

    let builtin = BuiltinFunction {
        name: "jalankan".to_string(),
        arity: Some(1),
        func: builtin_jalankan,
    };
    env.define("jalankan".to_string(), Value::Builtin(Rc::new(builtin)), true);

    let builtin = BuiltinFunction {
        name: "tutup".to_string(),
        arity: Some(1),
        func: builtin_tutup,
    };
    env.define("tutup".to_string(), Value::Builtin(Rc::new(builtin)), true);
}

fn builtin_serverweb_http(args: &[Value], span: &crate::error::Span) -> Result<Value, crate::error::Galat> {
    let port = match &args[0] {
        Value::Number(n) => *n as u16,
        _ => return Err(crate::error::Galat::runtime("Port ServerHttp harus berupa angka", span)),
    };

    let server_id = (port as u64) * 1000 + std::time::SystemTime::now()
    ...
    let handle = ServerHandle {
        ...
        rute_get: HashMap::new(),
        rute_post: HashMap::new(),
    ...
    map.insert("rute_get".to_string(), Value::Map(Rc::new(RefCell::new(HashMap::new()))));
    map.insert("rute_post".to_string(), Value::Map(Rc::new(RefCell::new(HashMap::new()))));

    Ok(Value::Map(Rc::new(RefCell::new(map))))
}

fn builtin_tambah_rute(args: &[Value], span: &crate::error::Span) -> Result<Value, crate::error::Galat> {
    let server_id = match &args[0] {
        Value::Map(m) => {
            let map = m.borrow();
            match map.get("_id") {
                Some(Value::Number(n)) => *n as u64,
                _ => return Err(crate::error::Galat::runtime("Server ID tidak valid", span)),
            }
        }
        _ => return Err(crate::error::Galat::runtime("Argumen pertama harus server handle", span)),
    };

    let method = match &args[1] {
        Value::String(s) => s.to_uppercase(),
        _ => return Err(crate::error::Galat::runtime("Method harus string", span)),
    };

    let path = match &args[2] {
        Value::String(s) => s.clone(),
        _ => return Err(crate::error::Galat::runtime("Path harus string", span)),
    };
    ...
    if let Ok(mut handles) = widya_server_handles().lock() {
        if let Some(handle) = handles.get_mut(&server_id) {
            match method.as_str() {
                "GET" => {
                    handle.rute_get.insert(path, handler);
                }
                "POST" => {
                    handle.rute_post.insert(path, handler);
                }
                _ => return Err(crate::error::Galat::runtime("Method GET|POST saja yang didukung", span)),
            }
        }
    }
...
fn builtin_jalankan(args: &[Value], _span: &crate::error::Span) -> Result<Value, crate::error::Galat> {
    let server_id = match &args[0] {
        Value::Map(m) => {
            let map = m.borrow();
            match map.get("_id") {
    ...
    if let Ok(mut handles) = widya_server_handles().lock() {
        if let Some(handle) = handles.get_mut(&server_id) {
            ...
            // TODO: implement WebServer integration
            // For now, just return Nil (blocking would require proper async/threading)
        }
    }
...
fn builtin_tutup(args: &[Value], _span: &crate::error::Span) -> Result<Value, crate::error::Galat> {
    let server_id = match &args[0] {
        Value::Map(m) => {
            let map = m.borrow();
            match map.get("_id") {
